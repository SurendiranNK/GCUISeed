...
function MainCtrl($http,$scope) {
    ...
    function getLeftdata(){
            ...
            $http({
                ...
                headers: {
                    ...
                    data:{'id':27,'method':"QueueProvService.getQueues",'params':[] }
            }).then(function(response) {
                 $scope.left_menu_data = response.data;
                 var data = $scope.left_menu_data;
                 $scope.left_menu_length = data.result.length;
            });

    }

    $scope.playMusic = function(music){
        if(music == "preatendedor" || music == "soporte_cola") {
            $scope.ShowHide(2);
             $http({
                method: 'POST',
                url: apiUrl,
                headers: {
                    ...
                    data:{'id':40,'method':"IVRProvService.getIVRSoundURL",'params':[music] }


            }).then(function(response) {
               //$scope.music = response.data.result;
                  for(var i = 0; i < response.data.result.length; i++){
                        var datas = response.data.result[i].URL;
                        if(i ==1){

                           $scope.aURL = response.data.result[i].URL ;
                            $scope.playNg2 (response.data.result[i].URL)
                        }
                        $scope.soundplay={"sound": $scope.aURL} 
                    }
            });
        }else{
              $scope.ShowHide(1);
    ...
    $scope.rowHighilited1 =function(row){
      $scope.selectedRow1= row;   

    }
    $scope.rowHighilited2 =function(row){
      $scope.selectedRow2= row;    
    }

    $scope.setRingMode=function(ringType){
       if(ringType=='SEQUENTIAL'){
          $("#sequencial_ringmode").attr('checked','checked'); 
          $scope.queue_ringMode='SEQUENTIAL';
       }else{
          $("#simultaeno_ringmode").attr('checked','checked'); 
          $scope.queue_ringMode='SIMULTANEOUS';
       }
    }

     $scope.getQueueLists = function(id){
        $scope.ID = id;
           //QueueProvService.getQueue
            $http({
                method: 'POST',
                url: apiUrl,
                headers: {
                    ...
                    data:{'id':25,'method':"QueueProvService.getQueue",'params':[id] }
            }).then(function(response) {
                $scope.queue_name = response.data.result.queue.name;
                $scope.queue_waitTimeOut = response.data.result.waitTimeOut;
                //console.log( $scope.queue_name+"---"+response.data.result.waitTimeOut)
                //$scope.queue_ringMode =response.data.result.ringMode;
                $scope.setRingMode(response.data.result.ringMode);
            ...
            $http({
                ...
                headers: {
                    ...
                    data:{'id':26,'method':"IVRProvService.getAudioFiles",'params':[] }
            }).then(function(response) {
        
            });

        //QueueProvService.getQueueTerminals
            $http({
                method: 'POST',
                url: apiUrl,
                headers: {
                    ...
                    data:{'id':25,'method':"QueueProvService.getQueueTerminals",'params':[id] }
            }).then(function(response) {
                $scope.inUse = response.data.result.inuse;
                $scope.available = response.data.result.available;

            });
            //QueueProvService.getQueueAudio
             $http({
                method: 'POST',
                url: apiUrl,
                headers: {
                    ...
                    data:{'id':28,'method':"QueueProvService.getQueueAudio",'params':[id] }
    

            }).then(function(response) {
                $scope.music = response.data.result.music;
            });
        }
...
/*
     $scope.availInfo = "";
     $scope.inUseInfo = "";*/
     $scope.putAvailValue = function(data,index){
        $scope.availInfo = data;
        $scope.availInfoindex = index;
/**/
     }
...
/*
$("#audio").empty();
$("<source>").attr("src","http://www.schillmania.com/projects/soundmanager2/demo/mpc/audio/CHINA_1.mp3").attr("type", "audio/mpeg").attr("id", "mp3").appendTo("#audio");
$("#audio").load();*/
    ...
    /*$scope.playAudio = function(id) {
     ...
     /*$scope.audioURL = "https://recordings-gs.anura.com.ar/AAFS/Actions/GetRecordingFile?Authorization=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJleHAiOjE0ODM3ODgzMzMsImNhbGxpZCI6MTA3MTQ3MzM3LCJ1c2VyaWQiOjQ1NDEsInRlbmFudGlkIjoyMDMxLCJpYXQiOjE0ODM3ODY1MzN9.E1rkECSNYmHEUSbuwbrwsZ-iXPASDFr4709WDkMWQ84&Format=mp3";*/
      $scope.playNg2 = function(url) {
                 var audio = new Audio(url);
                    audio.load();
                    audio.play();
     };

     $scope.setRingueo = function(ringType){
               var queue = {
                     "id":$scope.ID,
                     "name": $scope.queue_name,
               };
               var postdata={"queue":queue,"ringMode":ringType,"waitTimeOut":$scope.queue_waitTimeOut}
              /* $scope.dataPost.push(postdata);*/
            ...
            $http({
                ...
                headers: {
                    ...
                    data:{'id':28,'method':"QueueProvService.updateQueue",'params':[postdata]}

            }).then(function(response) {
                /*$scope.queue_name = $scope.queue_name;
                $scope.queue_waitTimeOut = $scope.queue_waitTimeOut;
*/
            });
    
     }
     $scope.tab1 = function(name,ringtype,outTime){
               var queue = {
                     "id":$scope.ID,
                     "name": name,
               };
               var postdata={"queue":queue,"ringMode":ringtype,"waitTimeOut":outTime}
               /*$scope.dataPost.push(postdata);*/
     ...
     $scope.playAudioFile = function(id){
        $http({
                method: 'POST',
                url: apiUrl,
                headers: {
                    ...
                    data:{'id':28,'method':"AdminService.getRecordingURL",'params':[id] }
                ...
                $scope.music = response.data.result;
                  for(var i = 0; i < response.data.result.length; i++){
                        var datas = response.data.result[i].URL;
                        if(i ==1){
                           $scope.audioURL = response.data.result[i].URL ;
                           $scope.playNg2 (response.data.result[i].URL);
     ...
     $scope.downloadAudio = function(){
          /*  $scope.audioURL = "http://www.schillmania.com/projects/soundmanager2/demo/mpc/audio/CHINA_1.mp3";
        a href = */
     }
    ...
    $scope.deleteNombre = function(id,indx){
   /*     alert(id)*/
            $http({
                method: 'POST',
                url: apiUrl,
                headers: {
                    ...
                    data:{id:29,method:"QueueProvService.delQueue",params:[$scope.ID]}
            }).then(function(response) {
                getLeftdata();
            });
     }

     $scope.setIndexNo = function(index){
        ...
        if(data == undefined || data == ""){
            ...
            $http({
                ...
                headers: {
                    ...
                    data:{id:12,method:"QueueProvService.addQueue",params:$scope.dataPost }
            }).then(function(response) {
                 $scope.dataPost =[];
                 section = 2;
                 $scope.getQueueLists(response.data.result);
            });
        }
        
     }
    $scope.movetoInUse = function(ID,index,data){
        ...
        if(index != undefined || index != ""){
           ...
           $http({
                ...
                headers: {
                    ...
                    data:{id:13,method:"QueueProvService.addTerminal2Queue",params:[ID,dataObj] }
            }).then(function(response) {
                 if(response.data.result === true) {
                    ...
                    $scope.inUse.push(dataObj);
                    $scope.available.splice( index, 1 ); 
                    $scope.availInfoindex = "";
     ...
     $scope.putInUseValue = function(data,index){
        console.log("putInUseValue====="+index)
        $scope.inUseInfo = data;
        $scope.inUseInfoindex = index;
     }

     $scope.movetoAvailalble = function(ID,index,data){ 
        if( index != undefined  || index !=""){
             var dataObj = {
                 id:data.id,
                 name:data.name,
                 timeout:data.timeout,
                 lag:data.lagputAvailValue
             };
           $http({
                method: 'POST',
                url: apiUrl,
                headers: {
                    ...
                    data:{'id':13,'method':"QueueProvService.delTerminalFromQueue",'params':[ID,data.id] }
            }).then(function(response) {
                if(response.data.result === true) {
                    $scope.inUseInfo = "";
                    ...
                    $scope.available.push(dataObj);
                    $scope.inUse.splice(index, 1 ); 
                    $scope.inUseInfoindex = "";
                }
            });
        }else{
                alert('Please select');
...
function datatablesCtrl($scope,DTOptionsBuilder,$filter,$http){
        ...
        var apiUrl = "http://api-gc.anura.com.ar:8090/GCAPI/JSON-RPC"
            $http({
                ...
                headers: {
                    ...
                    data:{'id':27,'method':"AdminService.getCdrs",'params':["","",null] }
            }).then(function(result) {

                var value = JSON.stringify(result.data);
                $scope.data = result.data;
                 if($scope.data.result != null){
                 var var_total_duration = 0;
                 var var_total_price = 0;
                    for(var i = 0; i < result.data.result.length; i++){
                        var datas = result.data.result[i];
                        var_total_duration += datas.duration;
                        var_total_price  += datas.price;
            
                    }
                    $scope.total_duration = var_total_duration;
                    $scope.total_price = var_total_price;
                }else{
    ...
    $scope.viewRecorded = function(data){
    }
    $scope.filterData = function(a,b,c){
        a = $("#date_desde").val();
        b = $("#date_hasta").val();
        ...
        $http({
                ...
                headers: {
                    ...
                    data:{'id':27,'method':"AdminService.getCdrs",'params':[a,b,c] }
            }).then(function(result) {
